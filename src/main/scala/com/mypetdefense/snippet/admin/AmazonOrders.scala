...
object AmazonOrders extends Loggable {
  ...
  def amazonOrderExport: Box[LiftResponse] = {
    exportAmazonOrder.currentValue flatMap { export =>
      ReportingService.exportAmazonOrders(export)
    }
  }
...
class AmazonOrders extends Loggable {
  ...
  def render = {
    ...
    "#export-results" #> {
      ...
      "#download" #> SHtml.ajaxSubmit("Download", () => JsCmds.RedirectTo(AmazonOrders.exportAmazonOrder.calcHref(generateExportQuery)))
    } &
