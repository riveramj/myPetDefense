...
object Agencies extends Loggable {
  ...
  def exportTotalSalesResponse: Box[LiftResponse] = {
    totalSalesExportMenu.currentValue flatMap { name => ReportingService.exportTotalSales(name) }
  }

  def exportRawSalesResponse: Box[LiftResponse] = {
    salesDataExportMenu.currentValue flatMap { name => ReportingService.exportRawSales(name) }
  }
  ...
  def agencyMtdYtdExport: Box[LiftResponse] = {
    mtdYtdExportMenu.currentValue flatMap { name => ReportingService.exportAgencyMtdYtdSales(name) }
  }
...
class Agencies extends Loggable {
  ...
  def generateAgencyData(agency: Agency, parentAgency: Box[Agency]): CssSel = {
    ...
    val activeCustomers = Agency
      .getAllChildrenCustomers(agency)
      .flatMap(_.subscription.obj)
...

    {
      ".name *" #> agency.name &
        ...
        ".actions .delete [onclick]" #> Confirm(
          s"Delete ${agency.name}? This will delete all members and coupons.",
          ajaxInvoke(deleteAgency(agency) _)
        ) &
        ".actions .sales-export [href]" #> Agencies.salesDataExportMenu.calcHref(agency.name.get) &
        ".actions .same-day-cancel [href]" #> Agencies.sameDayCancelExportMenu.calcHref(
          agency.name.get
        ) &
        ".actions .cancellation-export [href]" #> Agencies.cancellationExportMenu.calcHref(
          agency.name.get
        ) &
        ".actions .total-sales-export [href]" #> Agencies.totalSalesExportMenu.calcHref(
          agency.name.get
        ) &
        ".actions .month-to-date-sales-export [href]" #> Agencies.monthToDateExportMenu.calcHref(
          agency.name.get
        ) &
        ".actions .month-to-date-export [href]" #> Agencies.mtdYtdExportMenu.calcHref(
          agency.name.get
        )
    }
  }
