...
class ItemProduction extends Loggable {
  ...
  def createItemFromRecipe = {
    ...
    if(validateFields.isEmpty) {
      ...
      for {
        ...
        InventoryService.updateItemCount(inventoryItem, currentCount, currentCount - realCreateCount)
      }
  ...
  def itemRecipeDetailBindings(
    ...
    ".add-item-container" #> {
      ...
      ".create-item .add-item-part [onclick]" #> SHtml.ajaxInvoke(addItemToRecipe(finishedItem, detailsRenderer) _)
    }
  }
