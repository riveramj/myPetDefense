...
object TrackingUploadCSV extends Loggable {
  ...
  def parseLine(fieldList: Array[String], lineCount: Int, headerIndex: Map[Columns.Value, Int], lastRequiredColumn: Int): Box[TrackingInfo] = {
  ...
  def toTrackingInfo(fieldList: Array[String], lineCount: Int, headerIndex: Map[Columns.Value, Int]): Box[TrackingInfo] = {
    val recipient = Columns.cellValue(Columns.Recipient, headerIndex, fieldList).openOr("")
    val rawTrackingNumber = Columns.cellValue(Columns.TrackingNumber, headerIndex, fieldList).openOr("")
