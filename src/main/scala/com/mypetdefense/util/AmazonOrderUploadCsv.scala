...
object AmazonOrderUploadCsv extends Loggable {
  ...
  def createAmazonOrder(
    ...
    val quantityPurchased =
      Columns.cellInt(Columns.QuantityPurchased, headerIndex, fieldList).openOr(0)
    val productPrice = Columns.cellDouble(Columns.ProductPrice, headerIndex, fieldList).openOr(0d)
    val productDiscount =
      Columns.cellDouble(Columns.ProductDiscount, headerIndex, fieldList).openOr(0d)
